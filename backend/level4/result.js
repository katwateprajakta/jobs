jQuery(document).ready(function(){
	jQuery.ajax({
		url:'http://localhost/JavaScript/jobs/backend/level4/data.json',
	    type: 'get',
	    dataType: 'json',
	    async: false,
	    complete: function(data, textStatus, jqXHR)
	    {
			...
			for (var x = 0; x < car_ids.length; x++) {
				...
				rentals.push({'id': x+1, 'price':tot_price,'options':{'deductible_reduction':deductible_reduction} ,'commission':{'insurance_fee':insurance_fee,'assistance_fee':assistance_fee,'drivy_fee':drivy_fee}});		
			}
