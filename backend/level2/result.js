jQuery(document).ready(function(){
	jQuery.ajax({
		url:'http://localhost/JavaScript/jobs/backend/level2/data.json',
	    type: 'get',
	    dataType: 'json',
	    async: false,
	    complete: function(data, textStatus, jqXHR)
	    {
			...
			for (var x = 0; x < car_ids.length; x++) {
				if(arr_no_of_days[x] > 10){
					console.log("in if")
					discounted_price_per_day = arr_price_per_day[0] - ((50/100)*arr_price_per_day[0]);
				}
				else if(arr_no_of_days[x] > 4 && arr_no_of_days[x] <= 10){
					discounted_price_per_day = arr_price_per_day[0] - ((30/100)*arr_price_per_day[0]);
				}
				else if(arr_no_of_days[x] > 1 && arr_no_of_days[x] <= 4){
					discounted_price_per_day = arr_price_per_day[0] - ((10/100)*arr_price_per_day[0]);
				}
				else{
					discounted_price_per_day = arr_price_per_day[0];
				}

				var tot_days_cost = discounted_price_per_day * arr_no_of_days[x];
				var tot_dist_cost = arr_price_per_km[0] * arr_distance[x];
				tot_price = tot_days_cost + tot_dist_cost;
				rentals.push({'id': x+1, 'price':tot_price});		
			}
