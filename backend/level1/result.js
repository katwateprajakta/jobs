jQuery(document).ready(function(){
	jQuery.ajax({
		url:'http://localhost/JavaScript/jobs/backend/level1/data.json',
	    type: 'get',
	    dataType: 'json',
	    async: false,
	    complete: function(data, textStatus, jqXHR)
	    {
			...
			for (var x = 0; x < car_ids.length; x++) {
				var tot_days_cost = arr_price_per_day[x] * arr_no_of_days[x];
				var tot_dist_cost = arr_price_per_km[x] * arr_distance[x];
